...
│   ├── glimmer-update/        # 应用内检查更新：拉 releases.json、比版本、下载安装包并校验；安装交给壳
...
- **自动造词**：用户自己连着选出的两个词（不是整句路径里的），合起来不超过 4 个字、词库与用户词里都没有，
  ...
  同一段拼音里自选一个词之后剩下的部分走整句候选（`jidiaole` 选 挤、剩下 掉了 按空格）时，接缝处第一个词的转移按这次上屏的份数记
  （翻下去挑了非首选的整句才是双份，按空格接受首选与路径上别的词一样是一份普通的），
  ...
  `mgs` 选过 美国式 下次就是首选。这一级按句首 / 句中分桶数（`ChoicePosition`，记与查都看当时的上文），
  否则它压着上下文得分，`ba` 两处都只能出次数多的那个；切分里非末尾简拼少的优先（`kaifa` 按 `kai fa` 读的 开放 压过按 `kai f a` 读的 开放啊）。
  ...
  一次普通事件记 `TRANSITION_WEIGHT` 2 份（接受首选、整句路径上的词、从文本学的词都是），用户翻下去挑了非首选的记双份
  （`EXPLICIT_TRANSITION_WEIGHT` 4）：整句是模型自己算的，按空格接受会把它喂回模型形成回声，用户明确改选一次就要能压过去。
  这套份数与个人 n-gram 的绝对折扣是同一把尺：折扣 D = 2 正好扣光一次普通事件，所以一次误选抬不动首选、同一条接续来第二次才算数。选择次数在整句路径上的加分取对数并封顶（`viterbi::WEIGHT_CAP`），只管同音词偏好，不许它抬起拆分路径。
...
- **Server 进程**：`apps/windows/server`（package `glimmer-windows-server`，bin `glimmer-server`）。平台无关的 Engine 装配（`assembly`）与协议分派（`dispatch`）在 `crates/glimmer-server`，Linux 引擎进程共用；Windows 包只留传输（`ipc`）与自绘 UI（`ui`）。`dispatch::Router` 按 `SessionId` 分派多会话（Windows 一个 Server 服务多个应用进程，
...
- **TSF DLL**：`apps/windows/tsf`（package `glimmer-windows-tsf`，`cdylib`，产物 `glimmer_tsf.dll`，依赖官方 `windows` crate 的 COM `implement` 宏）。「引擎层」不是 Engine 而是连 Server 的**管道客户端** `EngineClient`（平台无关、可端到端测）；
  ...
  用 `ShellExecuteW`（`uiAccess=true` 的 exe 用 `CreateProcess` 报 740），与 DLL 同目录的 `glimmer-server.exe`；
  进程内 5 秒冷却 + 跨进程命名互斥体（`Local\GlimmerServerLaunch`）保证多个应用同时发现 Server 不在时只起一个。
