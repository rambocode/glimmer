...
- [ ] ★ 长句准确率（诊断与已做的两步见 `docs/notes/long-sentence.md`，2026-09-19）：句子越长整句全对越难（docs 集首选 40% → 8%），字准确率一直 70–77%。
  ...
  ③ 语言模型：语料补技术 / 新闻 / 小说、三元用回指近似、固定 λ 换成按前词定的回退（验收：每格候选 6 → 12 不该再变差，现在 29.3% → 28.0%）；
  ④ 回退换成按前词定（绝对折扣 + 接续概率）干净集 +0.8 但回放词 −12，等有更干净的词级尺子再看；⑤ 100 键以上每键 parse 到 10 ms（切分 DP 随长度涨），要增量化
