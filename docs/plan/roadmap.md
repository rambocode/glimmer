...
- [x] 数据二进制化（2026-09-05，`crates/glimmer-format`）：`.qj` 容器 = 头 + 分节表 + `META`（名称 / 许可证 / 署名）+ 原样落盘的内存数组，
...
- [x] bigram 语言模型（`glimmer-lm`，`LanguageModel` trait 注入）：`dict-convert bigram` 从中文维基 + LCCC 统计，`pack lm` 打成 `lm.qj` 随 bundle 打包，没有时退化为一元
...
- [x] bundle id 改为 `app.glimmer.inputmethod`（2026-09-06，域名 glimmer.app 注册后；`TISInputSourceID` 与连接名同步，旧 id `com.yenharvey.glimmer` 的输入源要在系统设置里删掉重加）
...
- [x] pkg 安装器（2026-09-06）：`bundle.sh --pkg` 出 `Glimmer-<版本>.pkg`（装 `/Library/Input Methods/`，关重定位，postinstall 以登录用户身份
  `glimmer-macos --register` 注册并启用输入源，欢迎页 / 许可证 / 结束页），版本号与构建号打包时注入，`uninstall.sh` 随包；
...
- [x] 短语层（2026-09-12）：常用词表是词典词头，不收 我的 / 好的 / 不知道 / 有没有 这类人整块打的组合；`dict-convert phrases` 从语料相邻两三词里挖
  （对话语料 ≥ 2000 次 + 边界规则，读音由成分词拼出）5400 条进基础词库，品牌词 微明 也进（`brand.tsv`）；起因与验收见 `docs/notes/phrase-layer.md`
...
- [x] 释义兜底（2026-09-06）：Core `GlossFiller` trait（与 `Predictor` 分开：独立线程、攒批、不丢请求），随包表查不到的词库词 / 云端词**上屏后**入队，
  `glimmer-predict::CloudGlossFiller` 攒 1.5 秒或 8 个词发一次（提示词与 gloss-gen 同源，只要当前学习语言），结果经 `Translator::learn` 进
  `glimmer-translate::PersonalGlossary`（`user-glossary-<语言>.tsv`，格式同随包表，可手改，`LayeredTranslator` 个人表优先），随 flush 落盘；
...
- [x] 输入统计（2026-09-06）：Core `UsageMeter` trait，每次上屏折成汉字 / 中文词（整句按切词数）/ 英文词 / 次数；
  `glimmer-learning::UsageStats` 按天记 `usage.tsv`，与输入日志无关（日志关掉或清空统计还在），随学习数据每 60 秒落盘；
...
- [x] 生词识别（2026-09-06）：Core `VocabularyTracker` trait + `Sense::fresh`；一条译词在用户上屏时出现在候选窗口里的轮次不到 3（`FRESH_UNTIL`）算生词，
  ...
  `glimmer-learning::VocabularyBook` 一个译词一行 `user-vocab.tsv`（语言 / 译词 / 看到轮次 / 上屏 / 用过 / 首见 / 末见），只记译词与次数
- [x] 学习词汇统计（2026-09-06）：偏好设置「统计」页词汇块：见过 / 看熟 / 上屏过 / 打出过 / 本周新见的译词数（`VocabularySummary`），
  ...
  `glimmer-translate::LevelTable` 读 `assets/levels/levels-{en,ja}.tsv`（`tools/corpus/levels.py` 生成），`VocabularyBook::with_levels`；等级不进候选窗口
- [ ] 可选复习功能
...
- [~] Linux IBus / Fcitx5（`apps/linux`，Docker 里端到端测过，未上真机桌面；细节见 `docs/design/architecture.md`「Linux：IBus / Fcitx」）
  ...
  - [x] Fcitx5 插件（C++ 薄壳 + Rust 静态库，复用 IBus 的会话状态机；Docker 里真 fcitx5 端到端测过）
  - [x] 发版包分开编：IBus 引擎在 Ubuntu 22.04 编（能装 22.04 / Debian 12），Fcitx5 插件在 24.04 编（要 Fcitx5 5.1）
- [ ] 配置同步
- [ ] 跨平台词库

## Phase 6 — 云联想

Core 定义 `Predictor` trait（与 `Translator` / `Learner` 同一模式），网络实现放独立 crate `glimmer-predict`，
...
- [x] `glimmer-predict`：`CloudPredictor`，async-openai 走 OpenAI 兼容接口，后台线程防抖 300 ms、缓存 64 条、超时 5 s
...
- [x] 配置：`~/Library/Application Support/Glimmer/config.toml` 的 `[predict]`，首次运行写模板；密钥可填配置或环境变量
...
- [x] 小 Transformer 实验分支（2026-09-08）：`glimmer-neural`（candle，Metal / Accelerate / CPU），用途「重排整句转换的前 6 条路径」；
