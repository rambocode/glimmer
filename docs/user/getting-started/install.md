...
1. 下载 `Glimmer-<版本>-Setup.exe`，双击安装，需要管理员权限。安装目录保持默认的 `C:\Program Files\Glimmer`，不要改到其他目录或其他盘：候选窗口要盖过开始菜单、任务栏搜索这类系统界面，靠的是 Windows 的 uiAccess 机制，系统只对装在 `%ProgramFiles%` 里的程序启用它，装到别处会静默失效。
...
升级时直接安装新版安装包，学习数据与设置均保留；macOS 与 Windows 也可以在「关于」页点「检查更新」→「下载并安装」，见 [偏好设置](../settings/preferences.md#检查更新)。卸载见 [卸载](../help/uninstall.md)。
