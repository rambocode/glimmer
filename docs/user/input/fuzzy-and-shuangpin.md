...
title: 模糊音与双拼
order: 3
description: 九条模糊音的开启方式与排序规则；小鹤、自然码、微软、搜狗、智能ABC五套双拼的开启方式与差异。
---
...
五套方案：小鹤、自然码、微软、搜狗、智能ABC。在「偏好设置 → 通用」选择，留空为全拼（配置文件写法依次为 `xiaohe` `ziranma` `microsoft` `sogou` `abc`）。开启 [五笔](wubi.md) 时双拼设置被忽略。

- 拼音行显示解出的全拼（`nihc` → `ni'hao`），学习按全拼记录，切回全拼后同样受益。
- 零声母：小鹤 / 自然码写 `aa` `ai` `ah`（a、ai、ang）一族；微软 / 搜狗写 `o` + 韵母键（`oa` `ol` `oh`）；智能ABC只能写 `o` + 韵母键（`oa` a、`oj` an、`ob` ou、`or` er）。
