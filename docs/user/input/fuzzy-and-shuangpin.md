...
六套方案：小鹤、自然码、微软、搜狗、小浪、智能ABC。在「偏好设置 → 通用」选择，不选为全拼（配置文件写法依次为 `xiaohe` `ziranma` `microsoft` `sogou` `xiaolang` `abc`）。
双拼与 [五笔](wubi.md) 可以同时开着，那就是下面的混输。
...
拼音怎么读和要不要用五笔是**两件事**，分开设置：

| 轴 | 可选 | 配置文件 |
|---|---|---|
| 拼音方案 | 全拼（缺省）／六套双拼／大千注音／关 | `[general] scheme`：留空或 `pinyin` / `xiaohe` / `ziranma` / `microsoft` / `sogou` / `xiaolang` / `abc` / `zhuyin` / `none` |
| 五笔 | 关（缺省）／86／98／新世纪 | `[general] wubi`：留空 / `86` / `98` / `xsj` |

三种组合：只打拼音（五笔关）、只打五笔（拼音方案选「关」）、两个都开就是混输。
