...
use glimmer_format::{Container, Kind, Metadata};
...
/// 环境变量 `GLIMMER_NEURAL_DTYPE=f32|f16` 可强制。
fn weight_dtype() -> DType {
    match std::env::var("GLIMMER_NEURAL_DTYPE").as_deref() {
        Ok("f16") => DType::F16,
        Ok("f32") => DType::F32,
...
mod tests {
    ...
    fn packed_file_scores_like_the_directory() {
        ...
        let out_dir = std::env::temp_dir().join("glimmer-neural-tests/packed");
...
mod latency {
    ...
    /// 延迟探针：前文 64 字 + 8 条约 8 字的候选，一次 batch。`cargo test --release -p glimmer-neural -- --ignored --nocapture`
    #[test]
    #[ignore]
    fn batch_latency() {
        let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../data/model");
