...
use glimmer_core::storage::{read_text_lossy, write_atomic_str};
use glimmer_core::{Language, Sense, Translation};
...
impl PersonalGlossary {
    ...
    fn save_to(&self, path: &Path) -> Result<(), GlossaryError> {
        let mut text = format!(
            "# 微明个人释义表（{}）：释义兜底从云端写来的，格式同随包释义表，可手改。词\\t[词性. ]译词[|读音]\n",
            self.language.code()
        );
...
mod tests {
    ...
    fn round_trips_through_the_file_in_glossary_format() {
        let dir = std::env::temp_dir().join(format!("glimmer-personal-{}", std::process::id()));
