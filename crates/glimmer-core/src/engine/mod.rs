...
use glimmer_dictionary::{Dictionary, Match, WordList};
...
pub struct Engine {
    ...
    /// 中文模式里给英文词与英文补全候选（中英混输，缺省开）；与英文模式（Caps Lock）的候选无关。
    mixed_english: bool,
...
/// 同一段拼音里连着选出来的（`glimmer` 选 青 再选 简）是「用户把它当一个词打」的强信号，两次就够；
/// 第一次可能是误选或偶然。
...
fn pattern_key(pattern: &[glimmer_dictionary::SyllablePattern<'_>]) -> String {
...
fn abbreviated_count(patterns: &[glimmer_dictionary::SyllablePattern<'_>]) -> usize {
    patterns
        .iter()
        .rev()
        .skip(1)
