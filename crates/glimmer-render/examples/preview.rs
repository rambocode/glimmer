//! 离线预览：`cargo run --release -p glimmer-render --example preview -- --out target/render-preview`
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "glimmer_render=debug".into()),
        )
        .init();
    let args = Args::parse();
...
fn probe() -> Frame {
    Frame {
        preedit: None,
        rows: vec![Row::plain(0, "微明 hello 🙂 日本語 骨直曜")],
