//! 导入词库：把用户给的文件（微明 TSV、Rime `.dict.yaml`、现成的 `.qj`）变成用户词库目录里的一个 `.qj`。
//!
...
use glimmer_format::{Container, Metadata};
...
mod tests {
    ...
    fn imports_tsv_and_rime_into_qj() {
        let dir = std::env::temp_dir().join(format!("glimmer-import-{}", std::process::id()));
