...
use glimmer_core::{InputLogEntry, InputLogger};
use serde::Serialize;
...
mod tests {
    use super::*;
    use glimmer_core::{CommitEntry, InputSource};

    #[test]
    fn writes_one_json_line_per_entry_with_a_timestamp() {
        let dir = std::env::temp_dir().join(format!("glimmer-input-log-{}", std::process::id()));
