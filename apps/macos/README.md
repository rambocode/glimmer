...
```bash
apps/macos/scripts/bundle.sh --pkg        # target/pkg/Glimmer-<版本>.pkg
```

pkg 装到 `/Library/Input Methods/`（需要管理员密码），装完 postinstall 以登录用户身份跑 `glimmer-macos --register`（注册、启用并切成当前输入源），
...
| 变量 | 作用 |
|---|---|
| `GLIMMER_SIGN_IDENTITY` | `Developer ID Application: …`，给 `.app` 签名并开 hardened runtime |
| `GLIMMER_INSTALLER_IDENTITY` | `Developer ID Installer: …`，给 pkg 签名 |
| `GLIMMER_NOTARY_PROFILE` | `xcrun notarytool store-credentials` 存的 profile 名，设了就公证并 staple |
...
```bash
/Library/Input\ Methods/Glimmer.app/Contents/Resources/uninstall.sh          # 保留学习数据与配置
/Library/Input\ Methods/Glimmer.app/Contents/Resources/uninstall.sh --purge  # 连数据、配置、日志一起删
```
