...
微明 Core 的命令行壳：不依赖任何平台 API，用来查候选、看排序、量性能、回放评测。改 Core 的任何东西都先在这里验证，再去碰输入法。

```bash
cargo run --release -p glimmer-cli -- kaifa            # 查一个拼音
cargo run --release -p glimmer-cli -- kaifa nihao      # 查几个
cargo run --release -p glimmer-cli                     # 交互模式
```
...
| `--language en\|ja` | 学习语言（也可用环境变量 `GLIMMER_LEARNING_LANGUAGE`） |
...
| `--config <路径>` | 配置文件，缺省与输入法共用 `~/Library/Application Support/Glimmer/config.toml`；测试时给一份 `[predict] enabled = false` 的，免得每次查询都等云端 |
...
| `--shuangpin xiaohe\|ziranma\|microsoft\|sogou\|xiaolang\|abc\|off` | 双拼方案，覆盖配置；`off` 强制全拼 |
...
```bash
cargo run --release -p glimmer-cli -- --typing jintianwanshangwomenquchifan
```

## 回放评测：`--replay`

读输入法记的输入日志（`~/Library/Application Support/Glimmer/input-log.jsonl`，每次上屏一行），把每行当时的键重新喂给引擎，看现在的排序会不会把当时选的词放在首选：

```bash
cargo run --release -p glimmer-cli -- --replay ~/Library/Application\ Support/Glimmer/input-log.jsonl --misses 20
```
...
终端里日志走 stderr，`RUST_LOG=debug` 看逐键细节；设置 `GLIMMER_LOG_DIR` 时再按天写文件。
