...
| 目录 | package | 产物 | 职责 |
| --- | --- | --- | --- |
| `server/` | `glimmer-windows-server` | `glimmer-server.exe` | 持有唯一的输入内核 `glimmer-core::Engine`，跑在所有应用进程之外 |
| `tsf/` | `glimmer-windows-tsf` | `glimmer_tsf.dll` | TSF 文本服务，被加载进每个应用进程，只做按键转发与文档写入（候选窗口由 Server 自绘） |

```
应用进程 A ── glimmer_tsf.dll ─┐
应用进程 B ── glimmer_tsf.dll ─┼─ 命名管道 \\.\pipe\glimmer ─▶ glimmer-server（唯一的 Engine）
应用进程 C ── glimmer_tsf.dll ─┘
```
...
状态无法共享、崩溃会连累宿主应用）。微明照 Weasel（WeaselServer + WeaselTSF）、水杉（Server 进程）的做法：
Engine 只此一份，跑在独立的 Server 进程；每个应用进程里的 TSF DLL 只做两件事——把系统按键翻成协议消息发来、
把 Server 回的候选画到候选窗口。

## 为什么是两个 package 而不是一个

两个产物的依赖集合刻意不同：DLL 只依赖 `glimmer-core`、`glimmer-platform` 与官方 `windows` crate（COM
...
- **Server 进程**（`server/`）：装配并持有 Engine（词库 / 语言模型 / 翻译 / 学习），按 `SessionId` 为每个
  ...
  对应会话。
- **IPC 协议**（`glimmer-platform::protocol`，两端共用）：`ClientMessage`（DLL → Server：开 / 关会话（带宿主 exe 名，
...
```bash
cargo check --target x86_64-pc-windows-gnu -p glimmer-windows-server -p glimmer-windows-tsf
cargo test -p glimmer-windows-server -p glimmer-windows-tsf
```
...
```bat
:: 1) 编译出 DLL 与 Server
cargo build -p glimmer-windows-tsf -p glimmer-windows-server

:: 2) 注册文本服务（改 HKEY_CLASSES_ROOT，图标写到 %ProgramData%\Glimmer\glimmer.ico，要管理员）
regsvr32 target\debug\glimmer_tsf.dll

:: 3) 起 Server（引擎在这里；没起时 DLL 吃掉字母键但没有候选，起来后下一键 / 下次聚焦自动重连）
cargo run -p glimmer-windows-server

:: 4) 在系统「语言 / 输入法」里应能看到「微明」，切到它，在任意输入框敲字
::    日志都在 %LOCALAPPDATA%\Glimmer\logs\：server.<日期>.log / tsf.<日期>.log / settings.<日期>.log（按天，留 7 天）

:: 反注册
regsvr32 /u target\debug\glimmer_tsf.dll
```

## 设置程序与 Windows App Runtime

`settings/`（`glimmer-settings.exe`）用 Windows Reactor（WinUI 3）画界面，是三个产物里唯一依赖 Windows App Runtime 的。
