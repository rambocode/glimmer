...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=GLIMMER_UIACCESS");
    if std::env::var_os("CARGO_CFG_WINDOWS").is_some() {
        let ui_access = std::env::var("GLIMMER_UIACCESS").map_or(true, |v| v != "0");
        if !ui_access {
            println!(
                "cargo:warning=GLIMMER_UIACCESS=0：Server 不带 uiAccess，候选窗在 UWP 宿主里可能被盖住"
            );
        }
        let manifest = new_manifest("Glimmer.Server")
