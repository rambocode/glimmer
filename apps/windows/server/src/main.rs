...
fn user_dir() -> Option<PathBuf> {
    glimmer_platform::dirs::user_dir()
}

fn config_path() -> Option<PathBuf> {
    glimmer_platform::dirs::config_path()
}
...
/// 三个进程共用的日志目录 `%LOCALAPPDATA%\Glimmer\logs`（见 `glimmer_platform::dirs`），这里顺手建出来。
...
/// 日志目录 `%LOCALAPPDATA%\Glimmer\logs` 给 AppContainer 应用（任务栏搜索 / 设置）写权限：
