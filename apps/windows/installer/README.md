...
`C:\Program Files\Glimmer`，注册文本服务，并设登录自启。对应 macOS 的 pkg。

## 安装布局

```
C:\Program Files\Glimmer\
    glimmer_tsf-<版本>.dll       TSF 文本服务（64 位；被加载进每个应用进程；按版本起名，见「升级」）
    glimmer_tsf-<版本>-x86.dll   同上的 32 位版（企业微信 / WPS / 32 位 QQ 这类 32 位应用只能加载它）
    glimmer-server.exe       输入内核 Server（跑在应用进程外）
    glimmer-settings.exe     设置界面
    Microsoft.UI.Xaml.dll …   设置程序自带的 Windows App Runtime（自包含部署，见下节；约 56 MB / 185 个文件）
    glimmer.ico              开始菜单 / 启动项快捷方式的图标（exe 里也嵌了一份）
    data\generated\           dict.qj / lm.qj / glossary-{en,ja,zh}.qj / english.tsv / dicts\*.qj
    assets\                   emoji\ levels\ sample\
```

Server 与设置程序按 **exe 相对**定位随包资源（`glimmer_platform::resources`）：装机时资源与 exe 同级，
开发时是仓库 `ime\`（exe 在 `target\{debug,release}\` 下往上三层）。相对写法两套布局一致，只有根不同。

用户数据仍在 `%APPDATA%\Glimmer`（config.toml、密钥 .env、学习数据、统计），三个进程的日志在 `%LOCALAPPDATA%\Glimmer\logs`（`server.` / `tsf.` / `settings.` 前缀，按天，留 7 天）；
卸载不动这些。图标由 `regsvr32` 写到 `%ProgramData%\Glimmer\glimmer.ico`（DLL 里 include_bytes 内嵌）。
...
`glimmer_tsf.dll` 被加载进每一个有文本框的应用进程，文件锁着覆盖不了；Inno 缺省的 `CloseApplications`
用 Restart Manager 找出所有占用者要求关闭——对输入法 DLL 就是「关掉一切」，所以关掉它（`CloseApplications=no`），改成：

- DLL **按版本起名并排装**（`DestName: glimmer_tsf-<版本>.dll`），新文件从不与旧文件撞名；
...
`glimmer.iss` 再整个目录装到 `{app}` 下、与 `glimmer-settings.exe` 同级。
...
脚本 release 构建三个产物、从 `apps\windows\server\Cargo.toml` 读版本、找 `ISCC.exe`、编 `glimmer.iss`，
成品在 `target\installer\Glimmer-<版本>-Setup.exe`。改了数据 / 脚本但二进制没变时加 `-SkipBuild`；`-Sign` 用自签证书签产物
（uiAccess 要求 Server 签名 + 装 Program Files）。

也可手动：`iscc /DAppVersion=0.1.0 apps\windows\installer\glimmer.iss`。
...
- **没证书的包**（CI 内测版）：先设 `$env:GLIMMER_UIACCESS = '0'` 再打，Server 不嵌 uiAccess——没签名的 exe 带 uiAccess=true 会起不来。
