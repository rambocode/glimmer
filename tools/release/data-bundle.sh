...
# 这个脚本把本机 data/generated/ 里输入法要随包的文件打成 glimmer-data.tar.gz，
# 把本地整句模型 data/model/model.qjm（训练仓库 ../train 导出三件套，tools/release/pack-model.sh 打成一个文件）原样上传，
# 把 LLM 生成的中间产物（续跑用的 JSONL）打成 glimmer-llm-intermediates.tar.gz，上传（覆盖）到 `data` Release。
#
...
PRODUCT_FILES=(dict.qj lm.qj glossary-en.qj glossary-ja.qj glossary-zh.qj english.tsv english-frequency.tsv wubi86.qj wubi98.qj wubixsj.qj)
...
# 模型单独一个文件：只重训模型时不用重传词库；CI 放到 data/model/，bundle.sh / glimmer.iss 见到就随包（.qjm 内部已是 fp16，不再压）
cp "$MODEL_FILE" "$OUT/model.qjm"
present=()
for f in "${LLM_FILES[@]}"; do [[ -f "data/generated/$f" ]] && present+=("$f"); done
if [[ ${#present[@]} -gt 0 ]]; then
  tar -czf "$OUT/glimmer-llm-intermediates.tar.gz" -C data/generated "${present[@]}"
fi
...
if ! gh release view data >/dev/null 2>&1; then
  gh release create data --prerelease --title "产品数据（CI 打包用）" \
    --notes "输入法随包的词库 / 语言模型 / 释义表（glimmer-data.tar.gz）、本地整句模型单文件（model.qjm）与 LLM 生成的续跑中间产物（glimmer-llm-intermediates.tar.gz）。滚动覆盖，不是软件版本。由 tools/release/data-bundle.sh 上传。"
else
  gh release edit data --notes "输入法随包的词库 / 语言模型 / 释义表（glimmer-data.tar.gz）、本地整句模型单文件（model.qjm）与 LLM 生成的续跑中间产物（glimmer-llm-intermediates.tar.gz）。滚动覆盖，不是软件版本。由 tools/release/data-bundle.sh 上传。"
fi
gh release upload data "$OUT"/*.tar.gz "$OUT/model.qjm" "$OUT/SHA256SUMS" --clobber
# 早先上传的三件套 tar 已被 model.qjm 取代
gh release delete-asset data glimmer-model.tar.gz --yes 2>/dev/null || true
