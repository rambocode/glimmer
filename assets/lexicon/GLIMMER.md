...
这是微明自己的词库来源，从「输入法字词库_分类整理版」数据包拷入（2026-09-05），只去掉了与 TSV 内容相同的 TXT 副本；
...
cargo run --release -p glimmer-dict-convert -- lexicon --emit-ambiguous data/generated/lexicon-ambiguous.txt
# 2. 多音字词交给 LLM 标读音（可中断续跑；结果 data/generated/pinyin-llm.jsonl，不进 git，发布时作为 Release 附件保存）。
#    这份定主读音。2026-09-22 之前的它是从已发词库反推的、每词一个读音，一直沿用：重标一遍时模型会把少数词的主读音改错（不对称 bu dui chen）。
#    2b. 多读音另标一份 pinyin-readings.jsonl：去掉声调后拼法不同的常用读法全列出并给占比（一行 yi hang 一行字 / yi xing 一行人，
#    主角 zhu jue / zhu jiao 这类口语读法也列）。lexicon --pinyin-readings 只在列表含主读音时采用，每个读音一条、按占比分词频。
#    2026-09-22 那份：7.2 万词先整批标一遍，词库词频 ≥ 200、含多音字的 1.6 万个 2–4 字常用词再用现在的提示词重标覆盖
cargo run --release -p glimmer-gloss-gen -- pinyin
cargo run --release -p glimmer-gloss-gen -- pinyin --words <词表> --out data/generated/pinyin-readings.jsonl
# 3. 用初版词库分词、统计语料词频（基础词库里语料一次都没切出来的词，如 一行 总被切成 一 / 行，按成分二元合成次数，下一遍 lexicon 才有真实词频）（语料在 data/corpus/，见 docs/design/landscape.md）
cargo run --release -p glimmer-dict-convert -- bigram data/corpus/*.txt
...
cargo run --release -p glimmer-dict-convert -- lexicon --pinyin data/generated/pinyin-llm.jsonl --pinyin-readings data/generated/pinyin-readings.jsonl --frequency data/generated/lm-unigram.tsv
cargo run --release -p glimmer-dict-convert -- bigram data/corpus/*.txt
...
cargo run --release -p glimmer-dict-convert -- mine data/corpus/*.txt
cp data/generated/oov-filtered.tsv assets/lexicon/mined_words.tsv
cargo run --release -p glimmer-dict-convert -- lexicon --pinyin data/generated/pinyin-llm.jsonl --pinyin-readings data/generated/pinyin-readings.jsonl --frequency data/generated/lm-unigram.tsv --extra-words assets/lexicon/mined_words.tsv
...
#     读音由成分词拼出不用再标；人工过一遍拷成 assets/lexicon/phrases.tsv，与 mined_words.tsv、brand.tsv（品牌词 微明）一起 --extra-words 并入，再重跑 bigram。
...
cargo run --release -p glimmer-dict-convert -- bigram --phrases assets/lexicon/phrases.tsv --phrases assets/lexicon/domain_words.tsv --phrases assets/lexicon/common_words.tsv --phrases assets/lexicon/04_internet_slang/slang_words.tsv --brand assets/lexicon/brand.tsv --brand assets/lexicon/mixed_words.tsv data/corpus/*.txt
...
# 4e. 中英混杂词（C盘 / B站 / U盘 / T恤 / yyds / 打call）：assets/lexicon/mixed_words.tsv（词\t次数\t拼音，字母音节 + 汉字拼音），
#     与 phrases / domain_words 一样走 lexicon --extra-words；语言模型走 bigram --brand 直接写一元（--phrases 的合成要成分词在语料里，C 不是 token）。
#     拉丁字母 / 数字自成一个音节、读音就是它的小写形式（tools/dict-convert/src/syllable.rs），给定读音以本表为准（U盘 读 you pan）。
#     2026-09-22 之前这条路是断的：Unihan 里字母没有读音，lexicon 把含字母的词整条丢掉（dropped），旧 dict.tsv 里那 15 条是手工插的
...
# 4g. 网络用语与聊天短语：assets/lexicon/04_internet_slang/slang_words.tsv（词\t次数\t拼音，汉字词；含字母的在 mixed_words.tsv）。
#     与 domain_words / common_words 一样走 lexicon --extra-words 与 bigram --phrases，4c 那两条命令里加上它。
#     候选表与收词标准见 04_internet_slang/README.md，做法与前后数字见 docs/notes/internet-slang.md
# 5. 英文词表
